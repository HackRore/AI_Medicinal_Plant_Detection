...
export default function PredictResult({ result, imageUrl }: { result: any; imageUrl: string }) {
  ...
  const confColor = (prediction?.confidence ?? 0) >= 80 ? "#4ade80" // Green-400
                  : (prediction?.confidence ?? 0) >= 50 ? "#fbbf24" // Amber-400
                  : "#ef4444"; // Red-500

  const toxBg    = (toxicity?.level_code ?? -1) === 0 ? "bg-green-500/10 text-green-400 border-green-500/20"
                  : (toxicity?.level_code ?? -1) === 1 ? "bg-amber-500/10 text-amber-400 border-amber-500/20"
  ...
  return (
    ...
    >
            ...
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-primary-400 font-mono text-xs uppercase tracking-[0.3em]">
                        {plant?.scientific_name ?? "Classification Pending"}
                    </p>
                </div>
      ...
      <div className="p-8 space-y-8">
            ...
            <div className="flex gap-6">
                {(prediction?.top3 ?? []).slice(1).map((t: any, i: number) => (
                    <div key={i} className="flex flex-col">
                        <span className="text-sm font-bold text-white mb-0.5">{t?.name ?? "N/A"}</span>
                        <div className="h-1 w-12 bg-gray-800 rounded-full overflow-hidden">
                            <div className="h-full bg-primary-500" style={{ width: `${t?.confidence ?? 0}%` }} />
                        </div>
                        <span className="text-[9px] text-gray-500 font-bold mt-1 uppercase">{t?.confidence ?? 0}%</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                ...
                <p className="text-sm text-gray-300 leading-relaxed font-medium">
                    {medicinal?.preparation ?? "No preparation data available."}
                </p>
        ...
        {(medicinal?.contraindications?.length ?? 0) > 0 && (
        ...
        <div className="pt-8 border-t border-white/5 text-center">
             <p className="text-xs text-gray-500 max-w-lg mx-auto leading-relaxed italic">
                 "{medicinal?.description ?? "A medicinal species under G9 study."}"
             </p>
             <div className="mt-4 flex items-center justify-center gap-2 text-[10px] font-black text-gray-600 uppercase tracking-widest">
                 <span>{plant?.family ?? "Botanical Family"}</span>
                 <span className="w-1 h-1 bg-gray-600 rounded-full" />
                 <span>Origin: {plant?.native_region ?? "Global Repository"}</span>
             </div>
        </div>
      </div>
    </motion.div>
